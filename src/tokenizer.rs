use crate::structs::{Statement, Env, DefaultTypes, DefaultTypes::Function};
...
pub fn parse_exp(ss: &str, env: &mut Env, sss: &Statement) -> Result<DefaultTypes, String> {
    ...
    if s.parse::<i32>().is_ok() {
        Ok(DefaultTypes::Int(s.parse::<i32>().expect("Something odd happened here")))
    } else if s.starts_with('"') && s.ends_with('"') {
        Ok(DefaultTypes::Str(String::from(&s[1..s.len() - 1]).replace("|_", " ").replace("|-", "\n")))
    } else if s == "true" {
        ...
        match dt {
            ...
            DefaultTypes::Table(tab) => {
                Ok(DefaultTypes::Table(tab))
            }
            _ => {
                Ok(dt)
            },
